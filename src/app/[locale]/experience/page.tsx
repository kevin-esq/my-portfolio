...
export default function ExperiencePage() {
  ...
  return (
    <div className="bg-background text-foreground min-h-screen">
      ...
      <section className="bg-slate-100 px-8 py-10 dark:bg-slate-800/50">
        <div className="mx-auto max-w-6xl text-center">
          ...
          {activeTab === "work" && (
            <div className="space-y-8">
              {workExperience.map((job, index) => (
                <div
                  key={job.id}
                  className={`rounded-2xl border border-slate-200 bg-white p-8 shadow-sm transition-all duration-300 hover:shadow-lg dark:border-slate-700 dark:bg-slate-800 ${isVisible ? "animate-fade-in-up" : "opacity-0"}`}
                ...
                >
                  <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
                    <div className="flex-1">
                      ...
                      <div className="mb-4 flex flex-col gap-4 text-white sm:flex-row sm:items-center">
                      ...
                      <ul className="space-y-2">
                        {job.achievements.map((ach, i) => (
                          <li key={i} className="flex items-start gap-2 text-white">
                            <div className="mt-2 h-2 w-2 flex-shrink-0 rounded-full bg-emerald-500 dark:bg-emerald-400" />
                            {ach}
                          </li>
                        ))}
                      </ul>

                      <h4 className="mt-6 mb-2 font-semibold text-black dark:text-slate-50">
                        {t("projectTech")}
                      </h4>
              ...
              {education.map((edu, index) => (
                ...
                >
                  <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
                    ...
                    <div className="flex-1">
                      <h3 className="mb-2 text-2xl font-bold text-slate-50">{edu.degree}</h3>
                      <div className="mb-4 flex flex-col gap-4 text-white sm:flex-row sm:items-center">
                        <div className="flex items-center gap-2">
                          <BriefcaseIcon className="h-4 w-4" />
                          {edu.institution}
                      ...
                      <p className="mb-4 text-white">{edu.description}</p>
                      <h4 className="mb-2 font-semibold text-black dark:text-slate-50">
                        {tCommon("highlights")}
                      </h4>
                      <ul className="space-y-2">
          ...
          {activeTab === "certifications" && (
            <div className="grid gap-8 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
              {certifications.map((cert, index) => (
                <div
                  key={cert.id}
                  className={`group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-300 hover:shadow-lg dark:border-slate-700 dark:bg-slate-800 ${isVisible ? "animate-fade-in-up" : "opacity-0"}`}
                  style={{ animationDelay: `${index * 150}ms` }}
                >
      ...
      <section className="bg-slate-100 px-8 py-20 dark:bg-slate-800/50">
        <div className="mx-auto mb-16 max-w-6xl text-center">
          ...
          <p className="text-muted-foreground text-xl">{t("skillsSubtitle")}</p>
        </div>
